...
1. 강결합 시스템은 프로세서 간 결합력이 강하고, 약결합 시스템은 프로세서 간 결합력이 약하다는데, 그 이유  
- 강결합 시스템
    - 네트워크로 연결된 모든 컴퓨터의 프로세서가 하나의 메모리를 공유
    - 속도가 빠름
    - 경쟁 → 결합 교환으로 해결
    - 하나의 시스템에 문제가 생기면 다른 시스템에도 영향
- 약결합 시스템
...
2. 디지털 신호가 아날로그 신호보다 효율적인 이유  
- 같은 통신 대역폭에 더 많은 사용자를 수용할 수 있음
    - 데이터를 압축해서 전송하기 때문 (4:1, 10:1)
- 디지털 신호가 아날로그 신호보다 명확하고 규칙적 (e.g. 잡음)

3. 고가용성 구성 유형 중 상시 대기를 가장 많이 사용하는 이유  
- 구현이 간단함
- 한 번에 하나의 스레드나 프로세스만이 공유 자원에 액세스하도록 보장하여 경합 조건이나 데이터 일관성 문제를 방지
- 성능 예측 가능
- 상호 인계, 컨커런트 액세스는 데드락이나 동기화 문제, 일관성 문제가 생길 가능성이 있음
- 상시 대기를 가장 많이 쓴다고 해서 꼭 상시 대기일 필요는 없기에 각각의 장단점을 비교하고 적절한 선택을 할 것
